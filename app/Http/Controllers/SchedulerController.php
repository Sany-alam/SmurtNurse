       ...
       //file_put_contents('test.txt','hello');
        ...
        echo json_encode($data);
            ...
            $duration_hour = ceil($duration_minute/60);
              ...
              //file_put_contents('test.txt',$nurse_prefered_date_time);
              $nurse_prefered_date_time = json_decode($nurse_prefered_date_time);
              //var_dump($nurse_prefered_date_time);
              //file_put_contents('test.txt',json_encode($nurse_prefered_date_time));

              for($j=0;$j<sizeof($nurse_prefered_date_time);+$j++)
              {
             ...
             //file_put_contents('test.txt',json_encode($prefered_date));
              for ($k=0;$k<sizeof($prefered_date);$k++) {
                  $busy_time = array();
                $tmp_date = date('d-m-Y', strtotime($prefered_date[$k]));
           ...
           //usort($data2,array($this,'my_sort'));
            ...
            for ($i = 0;$i<sizeof($nurse_list);$i++) {
              ...
              $assesment_type = $patient_profile->assesment_type;
              $patient_area = $patient_profile->country;
              $nurse_area = $nurse_profile->nurse_area;
              $nurse_area = explode(',',$nurse_area);
              $duration_minute = $nurse_list[$i]->duration;
              if($assesment_type =="primary")
              {
                  $duration_minute = $duration_minute+200;
